...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        if not name:
            ...
            res = "Enter Correct name (only letters and spaces allowed)"
            message1.configure(text=res)
...
def TrainImages():
    ...
    if not hasattr(cv2, 'face') or not hasattr(cv2.face, 'LBPHFaceRecognizer_create'):
        mess._show(title='OpenCV Error', message='cv2.face.LBPHFaceRecognizer_create not found. Please install opencv-contrib-python.')
        return
...
def getImagesAndLabels(path):
    # get the path of all the .jpg files in the folder
...
def TrackImages():
    ...
    assure_path_exists("StudentDetails/")
    # Robustly handle Treeview widget destruction
    try:
...
# Load the background image from the correct path
try:
...
clock = tk.Label(window, fg="#000000", bg="White", width=20, font=('comic', 15, ' bold '))
...
message1 = tk.Label(frame2, text="1)Take Images  >>>  2)Save Profile" ,bg="#87CEEB" ,fg="black"  ,width=39 ,height=1, activebackground = "#3ffc00" ,font=('comic', 15, ' bold '))
message1.place(x=7, y=230)

message = tk.Label(frame2, text="" ,bg="#87CEEB" ,fg="black"  ,width=39,height=1, activebackground = "#3ffc00" ,font=('comic', 16, ' bold '))
...
# Define a list of email domains or add a new domain if you want 
...
def send_email():
    ...
    from_email = "anuragsrivastava241529@gmail.com"  # Update with your email
    password = "tvwj lmke pnjv ficd"  # Update with your email password
