...
class EnhancedFaceDetector:
    ...
    def detect_faces_haar(self, gray):
        ...
        return [(x, y, w, h, 1.0) for x, y, w, h in faces]  # Add confidence of 1.0
    ...
    def assess_face_quality(self, face_img):
        ...
        if blur_score > 100:  # Not too blurry
            quality += 30
        if 50 < brightness < 200:  # Good brightness
            quality += 30
...
def tick():
    ...
    clock.config(text=f"{current_time} | FPS: {fps} | GPU: {gpu_usage:.1f}%")
    ...
    try:
        ...
        while True:
            ...
            for (x, y, w, h, confidence) in faces:
                ...
                if quality > 60:  # Quality threshold
                    sampleNum += 1
                    if quality > 80:
    ...
    try:
        while True:
            ...
            for (x, y, w, h, confidence) in faces:
                ...
                if quality > 50:  # Only process good quality faces
                    gray_face = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)
                    serial, conf, similarity = face_recognizer.predict(gray_face)
                    
                    name = "Unknown"
                    if conf < 50:  # Recognition threshold
                        try:
                    ...
                    # Display info
                    info_text = f"{name} ({similarity:.1f}%)"
                    cv2.putText(im, info_text, (x, y-10), font, 0.6, (255, 255, 255), 2)
...
window.title("🚀 GPU-Enhanced Face Recognition Attendance System")
...
clock = tk.Label(window, fg="#000000", bg="White", width=35, font=("comic", 12, " bold "))
clock.place(relx=0.45, rely=0.09)
tick()
...
def send_email():
    ...
    msg["Subject"] = f"Today's Attendance Report (GPU-Enhanced) - {date} - {time.strftime('%I:%M:%S %p')}"
