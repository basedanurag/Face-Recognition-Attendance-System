...
def change_pass():
    ...
    master.configure(background="#faf0e6")
    
    lbl4 = tk.Label(master, text="Enter Old Password:", bg="#faf0e6", fg="#5d4e75", font=("Helvetica", 12, " bold "))
    lbl4.place(x=10, y=20)
    global old
    old = tk.Entry(master, width=25, fg="#5d4e75", bg="#fff5ee", relief="solid", font=("Helvetica", 12, " bold "), show="*")
    old.place(x=200, y=20)
    
    lbl5 = tk.Label(master, text="Enter New Password:", bg="#faf0e6", fg="#5d4e75", font=("Helvetica", 12, " bold "))
    lbl5.place(x=10, y=60)
    global new
    new = tk.Entry(master, width=25, fg="#5d4e75", bg="#fff5ee", relief="solid", font=("Helvetica", 12, " bold "), show="*")
    new.place(x=200, y=60)
    
    lbl6 = tk.Label(master, text="Confirm New Password:", bg="#faf0e6", fg="#5d4e75", font=("Helvetica", 12, " bold "))
    lbl6.place(x=10, y=100)
    global nnew
    nnew = tk.Entry(master, width=25, fg="#5d4e75", bg="#fff5ee", relief="solid", font=("Helvetica", 12, " bold "), show="*")
    nnew.place(x=200, y=100)
    
    cancel = tk.Button(master, text="Cancel", command=master.destroy, fg="#d32f2f", bg="#ffcdd2", height=1, width=12, font=("Helvetica", 11, " bold "))
    cancel.place(x=250, y=150)
    save1 = tk.Button(master, text="Save", command=save_pass, fg="#2e7d32", bg="#c8e6c9", height=1, width=12, font=("Helvetica", 11, " bold "))
...
try:
    ...
    if bg_label is None:
        ...
        gradient_img = Image.new('RGBA', (win_width, win_height), (248, 249, 250, 255))
        gradient_draw = ImageDraw.Draw(gradient_img)
        
        # Add soft pastel gradient effect
        for i in range(win_height):
            # Soft lavender to light blue gradient
            progress = i / win_height
            r = int(248 - progress * 18)  # 248 -> 230
            g = int(249 - progress * 19)  # 249 -> 230
            b = int(250 - progress * 5)   # 250 -> 245
...
# Frame 1 (left) - Glass effect with subtle animation for profiles
...
# Create animated glass frame for left panel with pastel colors
...
message3 = tk.Label(title_bg, text=f"🚀 ENHANCED FACE RECOGNITION SYSTEM{gpu_info}", 
                   fg="#6b5b95", bg="#fff8dc", font=("Helvetica", 20, " bold "))
...
subtitle = tk.Label(title_bg, text="AI-Powered Attendance Management with GPU Acceleration", 
                   fg="#8b7d6b", bg="#fff8dc", font=("Helvetica", 12))
subtitle.place(relx=0.5, rely=0.5, anchor="center")

# Date and clock with pastel styling
date_frame = tk.Frame(title_bg, bg="#b8d4f1", relief="raised", bd=1)
date_frame.place(relx=0.05, rely=0.65, relwidth=0.25, relheight=0.25)

datef = tk.Label(date_frame, text=day + "-" + mont[month] + "-" + year, fg="#4a5568", bg="#b8d4f1",
                font=("Helvetica", 12, " bold "))
datef.place(relx=0.5, rely=0.5, anchor="center")

clock_frame = tk.Frame(title_bg, bg="#f7cac9", relief="raised", bd=1)
clock_frame.place(relx=0.70, rely=0.65, relwidth=0.25, relheight=0.25)

clock = tk.Label(clock_frame, fg="#4a5568", bg="#f7cac9", font=("Helvetica", 10, " bold "))
clock.place(relx=0.5, rely=0.5, anchor="center")
tick()

# Pastel frame headers
head1_frame = tk.Frame(frame1, bg="#c8e6c9", relief="raised", bd=2)
head1_frame.place(x=0, y=0, relwidth=1, height=50)

head1 = tk.Label(head1_frame, text="👥 REGISTERED PROFILES", 
                fg="#2e7d32", bg="#c8e6c9", font=("Helvetica", 14, " bold "))
head1.place(relx=0.5, rely=0.5, anchor="center")

head2_frame = tk.Frame(frame2, bg="#e1bee7", relief="raised", bd=2)
head2_frame.place(x=0, y=0, relwidth=1, height=50)

head2 = tk.Label(head2_frame, text="➕ NEW REGISTRATION", 
                fg="#6a1b9a", bg="#e1bee7", font=("Helvetica", 14, " bold "))
head2.place(relx=0.5, rely=0.5, anchor="center")

# Pastel input fields
input_frame = tk.Frame(frame2, bg="#faf0e6")
input_frame.place(x=20, y=70, relwidth=0.9, height=120)

lbl = tk.Label(input_frame, text="🆔 Enter ID:", fg="#5d4e75", bg="#faf0e6", font=("Helvetica", 12, " bold "))
lbl.place(x=10, y=10)

txt = tk.Entry(input_frame, width=30, fg="#5d4e75", bg="#fff5ee", font=("Helvetica", 12), relief="solid", bd=2)
txt.place(x=120, y=10)

lbl2 = tk.Label(input_frame, text="👤 Enter Name:", fg="#5d4e75", bg="#faf0e6", font=("Helvetica", 12, " bold "))
lbl2.place(x=10, y=50)

txt2 = tk.Entry(input_frame, width=30, fg="#5d4e75", bg="#fff5ee", font=("Helvetica", 12), relief="solid", bd=2)
txt2.place(x=120, y=50)

# Clear buttons with pastel styling
clearButton = tk.Button(input_frame, text="🗑️", command=clear, fg="#d84315", bg="#ffcc80", width=3, 
                       height=1, font=("Helvetica", 10, " bold "), relief="raised")
clearButton.place(x=400, y=10)

clearButton2 = tk.Button(input_frame, text="🗑️", command=clear2, fg="#d84315", bg="#ffcc80", width=3, 
                        height=1, font=("Helvetica", 10, " bold "), relief="raised")
clearButton2.place(x=400, y=50)

# Status messages with pastel styling
status_frame = tk.Frame(frame2, bg="#faf0e6")
status_frame.place(x=20, y=200, relwidth=0.9, height=80)

message1 = tk.Label(status_frame, text="1) Capture Images  >>> 2) Train Model", bg="#faf0e6", fg="#5d4e75", 
                   font=("Helvetica", 11, " bold "))
message1.place(x=10, y=10)

message = tk.Label(status_frame, text="", bg="#faf0e6", fg="#66bb6a", font=("Helvetica", 11, " bold "))
...
# Pastel buttons
button_frame = tk.Frame(frame2, bg="#faf0e6")
button_frame.place(x=20, y=300, relwidth=0.9, height=150)

takeImg = tk.Button(button_frame, text="📸 CAPTURE FACES", command=TakeImages, fg="#1976d2", bg="#bbdefb", 
                   width=35, height=2, font=("Helvetica", 12, " bold "), relief="raised", bd=3)
takeImg.place(x=10, y=10)

trainImg = tk.Button(button_frame, text="🧠 TRAIN MODEL", command=psw, fg="#7b1fa2", bg="#e1bee7", 
                    width=35, height=2, font=("Helvetica", 12, " bold "), relief="raised", bd=3)
trainImg.place(x=10, y=70)

# Enhanced Treeview with pastel styling
tree_frame = tk.Frame(frame1, bg="#fff8f0")
tree_frame.place(x=10, y=110, relwidth=0.95, height=280)

# Configure treeview style with pastel colors
style = ttk.Style()
style.theme_use("clam")
style.configure("Treeview", background="#fffbf0", foreground="#5d4e75", fieldbackground="#fffbf0")
style.configure("Treeview.Heading", background="#c8e6c9", foreground="#2e7d32", font=("Helvetica", 10, "bold"))
...
# Control buttons with pastel styling
control_frame = tk.Frame(frame1, bg="#fff8f0")
control_frame.place(x=10, y=60, relwidth=0.95, height=40)

trackImg = tk.Button(control_frame, text="🚀 START ATTENDANCE", command=TrackImages, fg="#2e7d32", bg="#c8e6c9", 
                    width=20, height=1, font=("Helvetica", 11, " bold "), relief="raised", bd=2)
trackImg.place(x=10, y=5)

thresholdButton = tk.Button(control_frame, text="⚙️ SETTINGS", command=adjust_threshold, fg="#f57f17", bg="#fff9c4", 
                           width=15, height=1, font=("Helvetica", 11, " bold "), relief="raised", bd=2)
thresholdButton.place(x=250, y=5)

# Control panel at bottom
bottom_frame = tk.Frame(frame1, bg="#fff8f0")
bottom_frame.place(x=10, y=400, relwidth=0.95, height=80)

# Menu bar with pastel styling
menubar = tk.Menu(window, relief="flat", bg="#f3e5f5", fg="#4a148c", font=("Helvetica", 10))
filemenu = tk.Menu(menubar, tearoff=0, bg="#f3e5f5", fg="#4a148c")
...
# Email functionality with pastel styling
email_frame = tk.Frame(bottom_frame, bg="#fff8f0")
email_frame.place(x=0, y=0, relwidth=1, height=80)

email_label = tk.Label(email_frame, text="📧 Email Reports:", fg="#5d4e75", bg="#fff8f0", font=("Helvetica", 10, "bold"))
email_label.place(x=10, y=10)

recipient_email_entry = tk.Entry(email_frame, width=20, fg="#5d4e75", bg="#fffbf0", font=("Helvetica", 9))
...
send_email_button = tk.Button(email_frame, text="📤 SEND", command=send_email, fg="#1976d2", bg="#bbdefb", 
...
delete_all_button = tk.Button(email_frame, text="🗑️ RESET ALL", command=delete_all_data, fg="#d32f2f", bg="#ffcdd2", 
